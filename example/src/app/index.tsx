...
const CursorSymbol = () => {
  ...
  return (
    <Animated.Text style={{ opacity: blinkAnim, fontSize: 16, color: "#8B5CF6", fontWeight: "bold" }}>
      ▊
    </Animated.Text>
  );
};
...
const CodeBlock = ({ lang, code, theme, showCursor }: { lang: string; code: string; theme: any; showCursor: boolean }) => {
  ...
  return (
    <View style={[styles.codeBlock, { backgroundColor: theme.backgroundSelected }]}>
      <View style={styles.codeBlockHeader}>
        <ThemedText type="code" style={styles.codeBlockLang}>
...
export default function HomeScreen() {
  ...
  const handleSend = (textToSend?: string) => {
    ...
    ws.onerror = (error) => {
      ...
      handleStreamError("Failed to connect to RAG server. Make sure the backend is running.");
    };
  ...
  const renderMessageContent = (content: string, isUser: boolean, isStreaming: boolean = false) => {
    if (isUser) {
      return (
        <ThemedText style={{ color: scheme === "dark" ? "#ffffff" : "#000000", fontSize: 15, lineHeight: 22 }}>
          {content}
        </ThemedText>
      );
    }
    ...
    if (!content) {
      return (
        <View style={{ flexDirection: "row", alignItems: "center", gap: 6, marginVertical: 4 }}>
          ...
          <ThemedText type="small" themeColor="textSecondary">Generating...</ThemedText>
        </View>
      );
    }
        ...
        <View key={index}>
          {blocks.map((block, blockIdx) => {
            ...
            if (headingMatch) {
              ...
              return (
                <Text key={blockIdx} style={{ fontSize, fontWeight, marginTop, marginBottom, color: theme.text }}>
              ...
              return (
                <View key={blockIdx} style={{ marginVertical: Spacing.one }}>
                  {lines.map((line, lineIdx) => {
                    ...
                    const showCursorHere = isStreaming && isLastBlock && isLastLine;
                    ...
                    if (numListMatch) {
                      ...
                      return (
                        <View key={lineIdx} style={styles.bulletRow}>
                          <Text style={[styles.bulletDot, { color: theme.text, fontWeight: "600" }]}>{num}.</Text>
                    ...
                    if (trimmedLine.startsWith("•") || trimmedLine.startsWith("-") || trimmedLine.startsWith("*")) {
                      ...
                      return (
                        <View key={lineIdx} style={styles.bulletRow}>
                          <Text style={[styles.bulletDot, { color: theme.text }]}>•</Text>
                          <Text style={[styles.bulletText, { color: theme.text }]}>
                    ...
                    return (
                      <Text key={lineIdx} style={[styles.paragraph, { color: theme.text, marginLeft: Spacing.four }]}>
            ...
            const lines = block.split("\n");
            return (
              <ThemedText key={blockIdx} style={styles.paragraph} themeColor="text">
                {lines.map((line, lineIdx) => {
                  const isLastLine = lineIdx === lines.length - 1;
                  const showCursorHere = isStreaming && isLastBlock && isLastLine;
                  return (
  ...
  return (
    <ThemedView style={styles.container}>
      ...
      <View style={[StyleSheet.absoluteFill, styles.glowContainer]}>
        <View style={[styles.glowCircle1, { backgroundColor: scheme === "dark" ? "rgba(139, 92, 246, 0.08)" : "rgba(139, 92, 246, 0.15)" }]} />
        <View style={[styles.glowCircle2, { backgroundColor: scheme === "dark" ? "rgba(20, 184, 166, 0.06)" : "rgba(20, 184, 166, 0.12)" }]} />
      </View>
      ...
      <SafeAreaView style={styles.safeArea} edges={["top", "left", "right"]}>
        ...
        <View style={styles.header}>
          <View style={styles.headerLeft}>
            ...
            <View>
              <ThemedText type="smallBold" style={styles.headerTitle}>Aether AI</ThemedText>
          ...
          {messages.length > 0 && (
            <TouchableOpacity style={styles.clearButton} onPress={handleClear} activeOpacity={0.7}>
              <ThemedText type="small" style={styles.clearText}>Clear Chat</ThemedText>
            </TouchableOpacity>
          )}
        </View>
        ...
        >
            ...
            <FlatList
              ...
              ListHeaderComponent={
                <View style={styles.emptyContainer}>
                  <Animated.View style={[styles.emptyAvatarGlow, { transform: [{ scale: pulseAnim }] }]}>
                  ...
                  <ThemedText themeColor="textSecondary" style={styles.emptySubtitleText}>
                    Ask questions, debug code, draft essays, or design beautiful layouts in real-time.
                  </ThemedText>
                  ...
                  <View style={styles.suggestionsContainer}>
                    {SUGGESTIONS.map((item, index) => (
                      <TouchableOpacity
                        key={index}
                        style={[styles.suggestionCard, { backgroundColor: theme.backgroundElement }]}
                        activeOpacity={0.8}
                      ...
                      >
                        ...
                        <ThemedText type="small" themeColor="textSecondary" style={styles.suggestionSub}>
                          {item.subtitle}
                        </ThemedText>
                      </TouchableOpacity>
                    ))}
                    ...
                    <View style={[styles.messageRow, styles.userRow]}>
                      <View
                        style={[
                          ...
                          styles.userBubble,
                          { backgroundColor: scheme === "dark" ? "#2F2F2F" : "#E2E8F0" },
                        ]}
                      >
                  ...
                  <View style={[styles.messageRow, styles.assistantRow]}>
                    <View
                      style={[
                        ...
                        {
                          backgroundColor: scheme === "dark" ? "#312E81" : "#E0E7FF",
                          borderColor: scheme === "dark" ? "rgba(99, 102, 241, 0.3)" : "rgba(99, 102, 241, 0.1)",
                        },
                      ]}
                    >
                      <Text style={{ fontSize: 13, color: scheme === "dark" ? "#E0E7FF" : "#4F46E5" }}>✨</Text>
                    </View>
          ...
          <View style={[styles.inputWrapper, { paddingBottom: Math.max(insets.bottom, Spacing.three) }]}>
            <View style={[styles.inputBar, { backgroundColor: theme.backgroundElement, borderColor: theme.backgroundSelected }]}>
              <TextInput
              ...
              <TouchableOpacity
                style={[
                  styles.sendButton,
                  { backgroundColor: inputText.trim() ? "#6366F1" : "rgba(99, 102, 241, 0.15)" },
                ]}
                activeOpacity={0.7}
                disabled={!inputText.trim()}
                onPress={() => handleSend()}
              >
                <ThemedText style={{ color: inputText.trim() ? "#ffffff" : theme.textSecondary, fontSize: 16, fontWeight: "bold" }}>
                  ↑
                </ThemedText>
              </TouchableOpacity>
            </View>
